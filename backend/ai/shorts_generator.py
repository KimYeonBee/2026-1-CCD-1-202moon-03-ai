...
[Step 1: 핵심 요약 + 캐릭터화(Characterization) 설정]

1. 내용 압축: 이 챕터에서 시청자가 반드시 기억해야 할 핵심 원리나 인과관계를 빠르고 타격감 있게 요약하라.
2. 개념의 캐릭터화: 추상적 개념을 의인화된 캐릭터로 변환하라. 각 캐릭터는 고유한 외모·성격·행동을 가져야 한다.
   캐릭터는 반드시 비유 세계 속 존재여야 한다. 교수/강사/학생 같은 현실 인물은 절대 금지.
   (예: 산화-환원 반응 → 빨간 갑옷의 "산화 기사"가 파란 망토의 "환원 마법사"에게서 전자 보석을 빼앗는 장면
        삼권분립 → 왕관 쓴 "입법 왕", 검 든 "사법 기사", 방패 든 "행정 장군"이 서로 견제하며 대치하는 장면
        수요-공급 → 배고픈 거인 "수요"가 빵집 주인 "공급"의 가게를 향해 돌진, 빵 가격표가 하늘로 솟구침)
3. 훅(Hook) 제거: 어그로를 끄는 서론은 버리고, 곧바로 캐릭터 행동으로 돌진하라.
...
Visual: 개념을 캐릭터화하여 행동으로 보여줘라. 캐릭터는 개념의 본질을 외모·의상·행동으로 표현하는 의인화된 존재다.
  ❌ 절대 금지: professor, teacher, lecturer, classroom, chalkboard, blackboard, whiteboard, podium, lecture hall, government official explaining, text, letters, words, subtitles, captions, signs, labels, numbers, writing, calligraphy, typography, watermark
  → 강의 장면과 모든 형태의 글자/텍스트는 절대 금지. 영상에 어떤 언어(한국어, 영어, 중국어 등)의 문자도 등장해서는 안 된다.
  ✅ 개념을 캐릭터로 만들어 행동시켜라. 캐릭터는 고유한 외모(갑옷, 색상, 체형)와 성격이 있어야 한다.
  구체적으로 포함할 것:
  - 캐릭터의 고유 외모, 의상, 색상 코드 (예: "A red-armored knight character with glowing eyes")
  - 캐릭터의 구체적 동작 — 개념의 작동 원리를 물리적 행동으로 표현 (예: "charges forward and snatches a glowing jewel from the blue wizard")
  - 배경/소품 — 개념의 세계관에 맞는 환경 (예: "in a volcanic arena with floating electron orbs")
  - ❌ 나쁜 예: "A professor explains oxidation-reduction using character metaphors"
  - ✅ 좋은 예: "A red-armored knight lunges at a blue-caped wizard, ripping a glowing electron gem from his staff in a volcanic arena"
  ★ CRITICAL: The video must contain ZERO text, words, letters, numbers, signs, labels, or any written language in any script (Korean, English, Chinese, Japanese, etc.). Pure visual storytelling only.
...
Style: 영상 톤을 명시하라. 반드시 "no text, no written language, no watermarks" 를 포함하라.
  - 예: "Bright saturated colors, 3D Pixar-style animation, comedic tone, no text, no written language, no watermarks" / "Cinematic lighting, stylized CG look, dramatic mood, no text, no written language, no watermarks"
...
10. ★★★ 영상에 어떤 형태의 글자/텍스트/문자도 절대 포함하지 마라. 한국어, 영어, 중국어, 일본어 등 모든 언어의 문자가 금지다. 간판, 자막, 워터마크, 라벨 등 글자가 보이는 모든 요소를 배제하라. 순수 비주얼 스토리텔링만 사용하라."""
